        ...
        if (queryIndex !== -1) {
            req
                ...
                .forEach((value) => {
                    ...
                    queries[key] = decodeURI(val) // 한글 디코딩 처리
                })
        }
        switch (endpoint) {
            case '/job':
                ...
                res.write(
                    JSON.stringify(profileArr.filter(({job}) => job.indexOf(queries.query) !== -1))
                );
                res.end()
                break;
            case '/mail':
                ...
                res.setHeader('Access-Control-Allow-Origin', '*')
                res.write(
                    JSON.stringify(profileArr.filter(({mail}) => mail.indexOf('naver') !== -1))
                );
                res.end()
                break;
            default:
